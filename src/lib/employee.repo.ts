...
function sanitizePayload(p: EmployeePayload): EmployeePayload {
  const out: EmployeePayload = {
    name: p.name.trim(),
    role: p.role,
  };
...
class EmployeeRepository {
  async getAllEmployees(
    ...
    if (raw && raw.success && raw.items && Array.isArray(raw.items)) {
      return {
        items: raw.items as Employee[],
        total: raw.total ?? 0,
  ...
  async update(
    ...
    try {
      const cleanPayload = sanitizePayload(payload as EmployeePayload);
      const res = await axiosInstance.put(`${base}/${id}`, cleanPayload);
      const raw = res?.data;
      const emp = extractEmployee(raw);
      if (!emp) {
